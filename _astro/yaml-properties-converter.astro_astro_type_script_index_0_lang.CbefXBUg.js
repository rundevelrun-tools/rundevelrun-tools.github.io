...
`),s={},n=[{indent:-1,container:{__root:s},key:"__root"}];for(const r of e){if(!r.trim()||r.trim().startsWith("#"))continue;const o=r.search(/\S/),c=r.trim();for(;n.length>1&&n[n.length-1].indent>=o;)n.pop();const i=n[n.length-1],a=i.container[i.key];if(c.startsWith("- ")){const p=c.slice(2).trim();Array.isArray(i.container[i.key])||(i.container[i.key]=[]),i.container[i.key].push(h(p))}else if(c.includes(":")){const p=c.indexOf(":"),d=c.slice(0,p).trim(),l=c.slice(p+1).trim();l===""||l==="|"||l===">"?(a[d]={},n.push({indent:o,container:a,key:d})):a[d]=h(l)}}return s}function E(t,e=""){const s=[];if(Array.isArray(t))t.forEach((n,r)=>{const o=`${e}[${r}]`;n!==null&&typeof n=="object"?s.push(...E(n,o)):s.push(`${o}=${n??""}`)});else if(t!==null&&typeof t=="object")for(const[n,r]of Object.entries(t)){const o=e?`${e}.${n}`:n;r!==null&&typeof r=="object"?s.push(...E(r,o)):s.push(`${o}=${r??""}`)}return s}function w(t){try{const e=A(t);return E(e).join(`
`)}catch(e){return`# Error: ${e.message}`}}function T(t){const e={};for(const s of t.split(`
`)){const n=s.trim();if(!n||n.startsWith("#")||n.startsWith("!"))continue;const r=n.indexOf("=");if(r<0)continue;const o=n.slice(0,r).trim(),c=n.slice(r+1).trim(),i=o.split(".").flatMap(l=>{const u=l.match(/^(.+?)\[(\d+)\]$/);return u?[u[1],u[2]]:[l]});let a=e;for(let l=0;l<i.length-1;l++){const u=i[l],b=i[l+1],x=/^\d+$/.test(b);a[u]===void 0&&(a[u]=x?[]:{}),a=a[u]}const p=i[i.length-1],d=h(c);Array.isArray(a)?a.push(d):a[p]=d}return e}function v(t,e=0){const s="  ".repeat(e);let n="";if(Array.isArray(t))for(const r of t)r!==null&&typeof r=="object"?n+=`${s}-
${v(r,e+1)}`:n+=`${s}- ${I(r)}
